...
def GetPubChemId(drug_features, new_column_name="pubchem_id"):
    ...
    for ind in tqdm(df.index):
        ...
        try:
            drug_features.loc[ind, new_column_name]= re.findall(r'\((.*?)\)', str(deriv))
        except:
            drug_features.loc[ind, new_column_name]= "-"
