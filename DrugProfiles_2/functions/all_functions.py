...
from drug_features import get_pubchem_id, run_manual_corrections
...
def ShowResponseCurves(
    df, plots_in_row, plots_in_column, x_columns, y_columns, start_index=0, indexes=[]
):
...
def FittingColumn(
    ...
    for i in tqdm(range(len(indexes))):
        ...
        try:
            r2_scores[i], fitting_parameters[i] = FitCurve(
...
def FittedData(
    ...
    default_param=True,
    print_info=False,
    min_drug_profiles=None,
):

    r2, fit_param = FittingColumn(
        df, df.index, x_columns, y_columns, fitting_function, default_param=True
    )
    df["fitting_r2"] = r2
    ...
    if print_info:
        ...
        if min_drug_profiles:
            drugs_10 = (
                df.groupby("DRUG_ID")
                .size()[df.groupby("DRUG_ID").size() > min_drug_profiles]
                .to_dict()
            )
...
def DataPreprocessing(
    ...
    """
    drug_curves = pd.read_csv(
        folder_with_original_data + "normalised_dose_response_data.csv"
    )
    ...
    df = FilteringCurves(
        drug_curves,
        filtering_scenario=filtering_scenario,
    ...
    df = FittedData(
        df,
        x_columns=conc_columns,
        y_columns=response_norm,
        fitting_function=fitting_function,
        default_param=True,
        print_info=print_progress_info,
        min_drug_profiles=10,
    )
    ...
    cell_features = pd.read_csv(
        folder_with_original_data + "Cell_Line_Features_PANCAN_simple_MOBEM.tsv",
        sep="\t",
    )
    ...
    if print_progress_info:
        ...
        print(
            "Number of unique CCL in filtered drug profiles:", df["COSMIC_ID"].nunique()
        )
        common_cells = set(np.array(cell_features.columns[1:], dtype="int")) & set(
            df["COSMIC_ID"].unique()
        )
    ...
    drug_features = pd.read_csv(
        folder_with_results + "drug_features_with_pubchem_properties_final.csv"
    )

    df = pd.merge(
        left=merged_df, right=drug_features, on="DRUG_ID", how="left"
...
def TrainTestSplit(
    ...
    if print_progress_info:
        ...
        print(
            "Train/test for drugs with 50 profiles:",
            train_df_50.shape,
            test_df_50.shape,
        )
